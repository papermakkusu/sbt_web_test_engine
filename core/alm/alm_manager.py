...
class ALMUrl:
    def __init__(self, ip, domain, project):
        self.__base = u'http://' + ip + u'/qcbin'
        # self.__base = "http://sbt-hpalm/qcbin/"
...
class ALMSession:
    def __init__(self, Logging, login='sbt-paltcev-pe', password='123456'):
        try:
    ...
    def get_attachments(self, ALMUrl, *args):
        if self.__headers["Cookie"] is not None:
        ...
        if self.__headers["Cookie"] is not None:
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    print("Get ALM function with errors, returned message is not XML")
                    # self.__Logging.error(u"[ALMSession] Get ALM function with errors, returned message is not XML", u"PATH:", ALMUrl.__getattr__(*args), u"HEADERS:", self.__headers, ET.ParseError.message)
                return int(r.status_code), None
            else:
                # self.__Logging.error(u"[ALMSession] Get ALM function with errors", r.status_code, r.reason, u"PATH:", ALMUrl.__getattr__(*args), u"HEADERS:", self.__headers)
    ...
    def add_file(self, ALMUrl, files, *args):
        if self.__headers["Cookie"] is not None:
            fileToSend = {}
            for file in files:
...
if __name__ == "__main__":
    url = ALMUrl(ip="sbt-hpalm", domain='PROCESSING', project='SmartVista')
    a = ALMSession(Logger, 'sbt-paltcev-pe', '123456')
    print(a.open(url))
    type = "GET"
    address = "/qcbin/api/domains/PROCESSING/projects/SmartVista/Testing HTTP/1.1"
    data = """"""
